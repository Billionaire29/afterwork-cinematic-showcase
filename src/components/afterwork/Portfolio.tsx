...
const Card = ({ p, i }: { p: Project; i: number }) => {
  ...
  return (
    <motion.a
      ...
      className={`group relative block glass rounded-[20px] overflow-hidden transition-all duration-500 hover:border-accent/60 hover:-translate-y-2 hover:shadow-[0_30px_80px_-20px_rgba(216,30,54,0.25)] ${
        isWide ? "md:col-span-3" : "md:col-span-2"
      }`}
      style={{ aspectRatio: isWide ? "16/10" : "4/5" }}
    >
      <div className="absolute top-4 left-4 z-20 w-9 h-9 rounded-full bg-background/80 backdrop-blur border border-ink/10 flex items-center justify-center overflow-hidden">
        <img src={p.favicon} alt="" className="w-5 h-5" onError={(e) => ((e.currentTarget.style.display = "none"))} />
      </div>

      <div className="absolute inset-0 overflow-hidden bg-ink/[0.02]">
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {p.tags.map((t) => (
                <span key={t} className="font-mono-ui text-[10px] uppercase tracking-widest px-3 py-1 rounded-full border border-ink/15 text-ink/70">
                  {t}
                </span>
              ))}
            </div>
          </div>
          <span className="shrink-0 inline-flex items-center gap-1.5 font-mono-ui text-[10px] uppercase tracking-widest text-accent border border-accent/40 rounded-full px-3 py-1.5 group-hover:bg-accent group-hover:text-background transition-colors">
    ...
    <section id="work" className="relative py-28 md:py-36 border-t border-ink/5 overflow-hidden">
      ...
      <div className="container relative">
        ...
        <p className="mt-10 font-mono-ui text-[11px] tracking-[0.18em] uppercase text-ink/40 flex items-center gap-2">
          <span className="text-accent">✳</span> All projects are live — click to visit
        </p>
      </div>
    </section>
  );
